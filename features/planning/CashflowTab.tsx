...
const CashflowTab: React.FC<CashflowTabProps> = ({ 
  ...
  const { currentSavings, projectToAge, bankInterestRate, additionalIncomes, withdrawals } = cashflowState;
  ...
  const currentAge = Math.floor((new Date().getTime() - new Date(profile.dob).getTime()) / (365.25 * 24 * 60 * 60 * 1000));
  ...
  const monthlyProjection = useMemo(() => {
    ...
    for (let m = 0; m < totalMonths; m++) {
      ...
      // Base cashflow
      let monthIncome = cashflowData.monthlySavings; 
      ...
      if (profile.children && profile.children.length > 0) {
        ...
        const monthlyUniCost = uniCost / 12; // Approximation for cashflow purposes
      ...
      // Fallback to 70% of current expenses if custom amount is not set (Matches RetirementTab logic)
      ...
      if (ageAtMonth >= retirementAge && baseRetirementExpense > 0) {
      ...
      // Calculate investment deduction (moved to retirement pot)
      let monthlyInvestmentAmount = 0;
      if (ageAtMonth < retirementAge) {
        if (profile.monthlyInvestmentAmount && toNum(profile.monthlyInvestmentAmount, 0) > 0) {
          ...
          monthlyInvestmentAmount = (cashflowData.monthlySavings * investmentPercent) / 100;
        }
    ...
    return projection;
  }, [cashflowData, currentSavings, projectToAge, additionalIncomes, withdrawals, bankInterestRate, profile, retirement]);
    ...
    <div className="p-5">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-5 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <h3 className="m-0 text-lg font-bold text-gray-800">➕ Additional Income / Savings</h3>
            ...
            {additionalIncomes.map((income) => (
              <div key={income.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 items-end">
                   ...
                   <button onClick={() => removeIncome(income.id)} className="mb-2 px-3 py-2 bg-red-500 text-white text-xs rounded hover:bg-red-600">Remove</button>
                </div>
                <div className="mt-2 p-2 bg-blue-50 rounded text-[11px] text-blue-800">
                   💡 <strong>Event:</strong> {income.type === 'onetime' ? 'One-time' : income.frequency} {fmtSGD(income.amount)} starting {monthNames[income.startMonth]} at age {income.startAge}
                </div>
              </div>
            ))}
          </div>
               ...
               <table className="w-full border-collapse text-xs min-w-[900px]">
                  <thead className="sticky top-0 z-10 shadow-sm">
                     <tr className="bg-gray-100 border-b-2 border-gray-300 text-gray-700">
                        ...
                        <th className="p-3 text-right font-bold text-emerald-700 bg-gray-100">Base Income</th>
                  ...
                  <tbody>
                     {monthlyProjection.slice(0, monthsToShow).map((row, idx) => (
                        <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                           ...
                           <td className="p-3 text-right text-emerald-600">{fmtSGD(row.baseIncome)}</td>
