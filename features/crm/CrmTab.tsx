...
const CrmTab: React.FC<CrmTabProps> = ({ 
  ...
  // Fetch ALL profiles to ensure the map is complete, regardless of client assignment
  useEffect(() => {
    const resolveNames = async () => {
        if (!supabase) return;
        
        try {
            // Fetch all profiles to ensure we have a comprehensive map
            // RLS usually allows reading profiles for name resolution
            ...
            if (data) {
                ...
                data.forEach(p => {
                    ...
                    // Robust Name Logic: Prefer Name > Email
                    if (p.name && p.name.trim() !== '') {
                        displayLabel = p.name;
                    } else if (p.email) {
                        displayLabel = p.email.split('@')[0];
                    }
            ...
            console.debug('Name resolution failed.');
        }
    };
    resolveNames();
  }, [clients.length]); // Re-run if client count changes (e.g. import)
  ...
  const availableAdvisors = useMemo(() => {
    ...
    clients.forEach(c => {
      ...
      if (ownerId) {
         ...
         if (!label) {
             // Fallback: If no profile found, try stored email or ID
             label = c._ownerEmail || `Advisor ${ownerId.substring(0, 4)}`;
         }
  ...
  // Determine if we should show the Advisor column
  ...
  const executeBulkDelete = async () => {
      ...
      try {
          ...
          if (selectedClientId && idsToDelete.includes(selectedClientId)) {
              newClient();
          }
    ...
    <div className="p-6 md:p-8 animate-fade-in pb-24 md:pb-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
         ...
         <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
             ...
             {/* Advisor Filter - Added based on request */}
             {availableAdvisors.length > 1 && (
               <select 
                 value={advisorFilter} 
                 onChange={(e) => setAdvisorFilter(e.target.value)} 
                 className="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm cursor-pointer shrink-0 max-w-[200px]"
               >
             ...
             <div className="hidden md:flex bg-white border border-slate-200 rounded-xl items-center p-1 shadow-sm ml-2 shrink-0">
                 <button onClick={() => setIsGrouped(!isGrouped)} className={`p-2 rounded-lg transition-all ${isGrouped ? 'bg-indigo-100 text-indigo-700 font-bold' : 'text-slate-400 hover:text-slate-600'}`} title="Group by Status">
                                       ...
                                       <React.Fragment key={statusKey}>
                                           ...
                                           <tr className="bg-slate-50/80">
                                               <td colSpan={showAdvisorCol ? 8 : 7} className="px-4 py-2 border-y border-slate-200/50">
                                                   <div className="flex items-center gap-2">
                                                       <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${config.bg} ${config.text}`}>
                                                           {config.label}
                                                       </span>
                                           ...
                                           {group.map(client => (
                                               <ClientRow 
                                                   ...
                                                   onToggle={() => handleToggleSelect(client.id)}
                                                   onClick={() => setActiveDetailClient(client)}
                                                   onStatusUpdate={handleStatusChange}
                                                   onWhatsApp={() => setActiveWhatsAppClient(client)}
                                                   onRecordSale={() => setActiveSaleClient(client)}
                                                   onLoadProfile={() => loadClient(client, true)}
                                                   onDelete={() => handleDeleteClientWrapper(client.id)}
                                                   canDelete={canDeleteClient}
                                                   advisorName={getAdvisorLabel(client)}
                                                   showAdvisor={showAdvisorCol}
                                               />
                                           ))}
                                       </React.Fragment>
                                   );
...
// Sub-component for Cleaner Table Rendering
const ClientRow: React.FC<{ 
    client: Client, 
    isSelected: boolean, 
    onToggle: () => void, 
    onClick: () => void,
    onStatusUpdate: (c: Client, s: ContactStatus) => void,
    onWhatsApp: () => void,
    onRecordSale: () => void,
    onLoadProfile: () => void,
    onDelete: () => void,
    ...
    return (
        <tr className={`hover:bg-slate-50 transition-colors group cursor-pointer ${isSelected ? 'bg-indigo-50/20' : ''}`} onClick={onClick}>
            ...
            {showAdvisor && (
                <td className="px-4 py-3">
                    <div className="flex items-center gap-2" title={advisorName}>
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${advisorName === 'Me' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-600'}`}>
                            {advisorName === 'Me' ? 'ME' : (advisorName?.charAt(0).toUpperCase() || '?')}
                        </div>
                        <span className="text-xs font-medium text-slate-600 truncate max-w-[100px]" title={advisorName?.includes('Advisor') ? `ID: ${client._ownerId || client.advisorId}` : advisorName}>
