...
const calculateDealMetrics = (c: Client) => {
  ...
  const autoTags: Array<{ label: string; color: string }> = [];
  ...
  if (c.profile.children && c.profile.children.length > 0)
    autoTags.push({ label: '👶 Family', color: 'bg-blue-50 text-blue-600' });
...
const CrmTab: React.FC<CrmTabProps> = (props) => {
  const { clients, loadClient, deleteClient, saveClient, onRefresh, newClient } = props;
  ...
  const updateActiveClient = (field: string, value: any, nested?: string) => {
    ...
    if (nested === 'profile') {
      ...
      updatedClient.appointments = {
        ...(updatedClient.appointments || { firstApptDate: null, nextFollowUpDate: null }),
        [field]: value,
      };
  ...
  const pipelineStats = useMemo(() => {
    ...
    enrichedClients.forEach((c) => {
      ...
      const label = STATUS_METRICS[statusKey as ContactStatus]?.label || String(statusKey);
    ...
    const chartData = Object.values(stageMap).sort((a, b) => order.indexOf(a.name) - order.indexOf(b.name));
      ...
      <div className="bg-white border-b border-gray-200 p-4 flex flex-wrap gap-6 items-center shadow-sm z-20">
        <div className="flex-1 min-w-[200px]">
          <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
            CRM <span className="text-gray-400 font-light">|</span>{' '}
            <span className="text-sm font-medium text-gray-500">Pipeline View</span>
          </h2>
        </div>
              ...
              >
                ...
                {Object.keys(STATUS_METRICS).map((k) => (
                  <option key={k} value={k}>
                    {STATUS_METRICS[k as ContactStatus].label}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex-1 overflow-auto">
              <table className="w-full text-left border-collapse min-w-[1200px]">
                <thead className="bg-gray-50 sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-r border-gray-200 w-[220px]">
                      Client
                    </th>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-r border-gray-200 w-[140px]">
                      Status
                    </th>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-r border-gray-200 w-[120px] text-right">
                      Deal Value
                    </th>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-r border-gray-200 w-[100px] text-right">
                      Prob %
                    </th>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-r border-gray-200 w-[150px]">
                      Smart Tags
                    </th>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-r border-gray-200 w-[120px]">
                      Last Touch
                    </th>
                    <th className="p-3 text-[10px] font-extrabold text-gray-400 uppercase border-b border-gray-200">
                      Source
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="p-3 border-r border-gray-100 text-right">
                        <span
                          className={`text-xs px-2 py-0.5 rounded ${
                            client.metrics.probability > 0.5 ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                          }`}
                        >
                      ...
                      <td className="p-3 border-r border-gray-100">
                        <div className="flex items-center gap-2">
                          {client.metrics.isStale && (
                            <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" title="Stale Lead" />
                          )}
        ...
        {drawerOpen && activeClient && (
          <div className="absolute inset-y-0 right-0 w-full md:w-[450px] bg-white shadow-2xl border-l border-gray-200 transform transition-transform duration-300 ease-in-out z-30 flex flex-col">
            ...
            <div className="bg-gray-50 p-4 border-b border-gray-200 flex justify-between items-start">
              ...
              <button onClick={handleCloseDrawer} className="text-gray-400 hover:text-gray-600 text-xl">
                ✕
              </button>
            </div>
              ...
              <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-lg font-bold text-emerald-600">{fmtSGD(calculateDealMetrics(activeClient).potentialRevenue)}</div>
                  </div>
                </div>
              </div>
                    ...
                    <div>
                      ...
                      >
                        {Object.keys(SOURCE_CONFIG).map((k) => (
                          <option key={k} value={k}>
                            {SOURCE_CONFIG[k as LeadSource].label}
                          </option>
                        ))}
                      </select>
                    </div>
