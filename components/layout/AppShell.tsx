...
const AppShell: React.FC<AppShellProps> = ({ 
  ...
  const handleTabChange = (tabId: string) => {
    if (canAccessTab(user, tabId)) setActiveTab(tabId); else onPricingClick();
  };
  ...
  const handleSaveProfile = async () => {
      ...
      } catch (e) { alert("Failed to update profile."); } finally { setIsSavingProfile(false); }
  };
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 font-sans text-slate-900 overscroll-none">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative w-full overscroll-none">
         <header className="h-16 px-4 md:px-6 border-b border-gray-200 bg-white/80 backdrop-blur-md flex items-center justify-between z-20 shrink-0">
            <div className="flex items-center gap-3 md:gap-4">
               ...
               <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                     {clientRef ? (
                        <>
                           <h2 className="text-sm font-bold text-slate-900 truncate max-w-[150px]">{clientName || 'Unnamed Client'}</h2>
                           <span className="text-[10px] font-mono text-slate-400 px-1.5 py-0.5 bg-slate-100 rounded hidden sm:inline-block">DRAFT</span>
                        </>
            ...
            <div className="flex items-center gap-2 md:gap-3">
               <div className="flex flex-col items-end mr-1 md:mr-2 min-w-[110px]">
                  {saveStatus === 'saving' && (
               ...
               <button 
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all shadow-sm active:scale-95 ${saveStatus === 'saving' ? 'bg-slate-100 text-slate-400' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
               >
                  <span>{saveStatus === 'saving' ? '⏳' : '💾'}</span> <span className="hidden sm:inline">Push Changes</span>
               </button>
                  ...
                  <div className="relative" ref={profileRef}>
                     ...
                     {isProfileMenuOpen && (
                        <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-50">
                           ...
                           <button onClick={() => { setEditName(user.name || ''); setIsEditProfileOpen(true); setIsProfileMenuOpen(false); }} className="w-full text-left px-4 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 flex items-center gap-2 transition-colors">👤 Edit Profile</button>
                           <button onClick={() => { signOut(); setIsProfileMenuOpen(false); }} className="w-full text-left px-4 py-2 text-xs font-bold text-red-500 hover:bg-red-50 flex items-center gap-2 transition-colors">🚪 Sign Out</button>
                        </div>
                     )}
                  </div>
               )}
      ...
      <Modal isOpen={isEditProfileOpen} onClose={() => setIsEditProfileOpen(false)} title="Edit My Profile">
          <div className="space-y-4">
              <input className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm font-bold text-slate-900 outline-none" value={editName} onChange={(e) => setEditName(e.target.value)} />
              <div className="flex justify-end gap-2 pt-2">
                  <Button variant="ghost" onClick={() => setIsEditProfileOpen(false)}>Cancel</Button>
                  <Button variant="primary" onClick={handleSaveProfile} isLoading={isSavingProfile}>Save Changes</Button>
              </div>
          </div>
      </Modal>
    </div>
